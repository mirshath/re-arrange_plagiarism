...
if (!empty($studentIds)) {
    ...
    //     } else {
    //         // Allocate the checker to the student
    ...
    foreach ($studentIds as $studentId) {
        ...
        if (mysqli_num_rows($checkResult) > 0) {
            ...
            // Allocate the checker to the student, now with student_reg_id
            $allocateQuery = "INSERT INTO allocate_checker (student_id, student_reg_id, checker_id) VALUES ('$studentId', '$studentRegId', '$checkerId')";
            if (mysqli_query($conn, $allocateQuery)) {
                $checkerAllocations[$checkerId][] = $studentId;
                $response['messages'][] = "Student $studentId allocated to Checker $checkerId";
            } else {
